...
- **13 figure types** — multi-spectrum lines, scatter with optional OLS, hexbin density, error bars, heatmap, filled contour, matrix heatmap, polar response, histogram, box with raw points, correlation matrix, data table, flow diagram.
- **Export** — 300 dpi PNG, editable SVG, PDF; plus a reproducible ZIP containing the full imported table, drawing parameters, version record, SHA-256 checksum, a standalone Python script and all three image formats.
- **Reference knowledge base** — 25 bibliographic records and 108 figure/subfigure/table structure cases, each with required data columns, figure number, implementation recipe and provenance. 7 cases were visually checked against the original figure; the rest are used at their stated evidence level.
...
See [architecture](docs/architecture.md), [contributing](CONTRIBUTING.md), [validation](docs/validation.md) and [literature provenance](docs/literature.md).
...
- **13 类画法**：多光谱曲线、散点与可选线性拟合、六边形密度图、误差棒、热图、等高线、矩阵热图、极坐标、直方图、箱线与原始点、相关矩阵、数据表、流程关系图。
- **GUI**：多候选预览、推荐理由、数据检查、列映射、交互缩放平移、单栏/双栏/汇报尺寸、论文案例搜索。
- **来源知识库**：25 篇题录、108 条图/子图/表格结构案例，含数据列要求、图号、实现配方和出处；7 条经过原图视觉复核，其余按各自图注/图题证据级别使用。
- **导出**：300 dpi PNG、可编辑 SVG、PDF；ZIP 包含完整导入表、绘图参数、版本记录、SHA-256、独立运行的 Python 脚本和三种图片。
...
详见 [架构](docs/architecture.md)、[贡献说明](CONTRIBUTING.md) 和 [验证记录](docs/validation.md)。
