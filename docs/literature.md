...
- `catalog/styles.json`：图型子类库，只含 `label` / `patterns` / `data_schema` / `recipe`。**逐条出处字段已移除**——它们不参与推荐计算，也不应出现在输出图上。出处信息改由下面的题录在本页和项目 README 层面承担。
- `research/collect_openalex.py`：游标分页、缓存、重试、去重的题录采集器；只生成候选文件，不修改人工案例库。开发期工具，不随产品运行时使用。

下表记录的是**当初建立图型集合时**每条来源的可信程度。它描述调研过程的严谨性，不再是产品数据的一部分。
...
另对 3 篇论文各一页代表性原图进行了视觉复核，覆盖 7 条案例。复核纠正了损耗比曲线误归为热图，以及倒数坐标的变量定义。其余案例仍按上述证据级别使用，不视为已逐图审美审查。
...
- `--output` 可以指定候选文件，但禁止覆盖人工维护的 `papers.json` 和 `styles.json`。
...
已核对 `styles.json` 的唯一 ID、必需字段（`label` / `patterns` / `data_schema` / `recipe` 非空），并断言每个 `patterns` 值都对应 `render.FIGURE_TYPES` 中真实存在的渲染分支；同时断言七类逐条出处字段没有回潮。`papers.json` 核对唯一 ID 与题录完整性。**案例与论文之间的外键校验已随出处字段一并移除**，因此本页题录表中的案例数是历史记录，不再由测试保证。采集器做了离线缓存分页/去重验证，实际网络采集未作为完成项：本机直接联网请求被沙箱阻止，自动批准审核又因审核服务模型配置错误而拒绝；文献检索改用可用的网页工具完成。本版本未保存 OpenAlex 在线采集结果，也没有因此伪造数据量。
